...
namespace KHInsiderDownloader
{
    public partial class Form1 : Form
    {
        ...
        string defaultAlbumName = "Custom Album Name";
        ...
        private void UpdateLocalDownloadPathLabel()
        {
            ...
            if (!string.IsNullOrWhiteSpace(customAlbum) && customAlbum != defaultAlbumName) // User defined
            {
                completeDownloadFolderPath = $"{baseDownloadFolderPath}\\{customAlbum}";
            }
            else if (!string.IsNullOrWhiteSpace(possibleAlbumName)) // From page descriptor
            {
                completeDownloadFolderPath = $"{baseDownloadFolderPath}\\{possibleAlbumName}";
            }
            else if (!string.IsNullOrWhiteSpace(urlAlbumName)) // From URL
            {
        ...
        void UpdateCustomAlbumTextColor()
        {
            string fieldText = customAlbumTextBox.Text;
            customAlbumTextBox.ForeColor = (fieldText != defaultAlbumName) ? Color.Black : Color.Gray;
        }
        ...
        private void customAlbumTextBox_Enter(object sender, EventArgs e)
        {
            TextBox field = (sender as TextBox);
            if (field.Text == defaultAlbumName)
            {
                field.Clear();
            }
            else
        ...
        private void customAlbumTextBox_Leave(object sender, EventArgs e)
        {
            TextBox field = (sender as TextBox);
            if (string.IsNullOrEmpty(field.Text))
            {
                field.Text = defaultAlbumName;
            }
            UpdateCustomAlbumTextColor();
        }
    }
